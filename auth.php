...
// The user has not been marked as banned before.
// Try the login.
if ($user == "" OR $pass == ""){
	...
	// check if the user has any outstanding sessions before trying to make a new one.
	$get_existing = mysql_query("SELECT * FROM app_sessions WHERE ip = '".$_SERVER['REMOTE_ADDR']."' AND expires <= '".time()."'") or die(mysql_error());
	if (mysql_num_rows($get_existing) >= 1){
		// The user has a previous session.
		...
		if (mysql_num_rows($run)==0){
			...
			// Insert the session into a database with the last request time.
			// This key will be valid for a total of 60 minutes
			// 255 characters key to be safe.
			$key = "";
			...
			mysql_query("INSERT INTO app_sessions(ip, lid, hash, expires) VALUES('".$_SERVER['REMOTE_ADDR']."','".$aid[1]."','".$key."','".(time() - 3600)."')") or die(mysql_error());
			echo $key;
		}	
	}
}
