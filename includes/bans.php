...
class Bans{
	...
	function addStrike($ip){
		...
		if (mysql_num_rows($getCount) >= 3){
			mysql_query("INSERT INTO bans(ip,time,expires) VALUES('".$ip."', '".time()."', '".(time() + 3600)."')");
	...
	function addBan($ip, $time = 0){
		if ($ip == ""){ return false; }
		mysql_query("INSERT INTO bans(ip,time,expires) VALUES('".mysql_real_escape_string($ip)."', '".time()."', '".(time() + 3600)."')");
		return true;
	}
